...
function safeRemoteImageUrl(value) {
  ...
  return /^(cloud:\/\/|https:\/\/)/.test(value) ? value : ''
}
...
function extractSafePayload(collection, record, ownerKey) {
  ...
  if (collection === 'items') {
    ...
    safeRecord.stickerImageCloudFileId = record.stickerImageCloudFileId || ''
    
    // P2.8 Sync Timeline and legacy image URL compatibility
    safeRecord.timeline = Array.isArray(record.timeline) ? record.timeline : []
    safeRecord.originalImageUrl = safeRemoteImageUrl(record.originalImageUrl)
    safeRecord.displayImageUrl = safeRemoteImageUrl(record.displayImageUrl)
